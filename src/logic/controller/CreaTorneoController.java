...
import logic.bean.CreaTorneoBean;
...
public class CreaTorneoController implements Initializable {
    ...
    public void conferma() throws SQLException {
        ...
        if(dataDP.getValue().isBefore(datascadenzaDP.getValue())){
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setTitle("ERRORE DATA");
            alert.setContentText("La data del torneo � pi� recente della data di scadenza dell'iscrizione.");
            alert.showAndWait();
            return;
        }
        ...
        if(minPart<2){
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setTitle("ERRORE NUMERO MINIMO ISCRITTI");
            alert.setContentText("Il numero minimo di partecipanti � 2.");
            alert.showAndWait();
            return;
        }
